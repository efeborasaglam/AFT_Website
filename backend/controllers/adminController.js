...
const addDoctor = async (req, res) => {
    try {
        ...
        // parse speciality (comes in as JSON string array, e.g. '["Online-Coaching","Athletik-Gruppentraining"]')
        const parsedSpeciality = JSON.parse(speciality)
        if (!Array.isArray(parsedSpeciality) || parsedSpeciality.length === 0) {
            return res.json({success: false, message: "Select at least one speciality"})
        }
...
const allDoctors = async (req, res) => {
    try{
        ...
        res.json({success: true, message: "Trainers List", doctors})
    }catch (error){
        console.log(error)
        res.json({success: false, message: error.message + "Something went wrong"})
    }
